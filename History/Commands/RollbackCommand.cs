	...
	public class RollbackCommand : HCommand
	{
		...
		public override void Execute()
		{
			...
			using (QueryResult reader =
				History.Database.QueryReader("SELECT * FROM build_history WHERE Account = @0 AND Time >= @1 AND " + XYReq + " AND WorldID = @2",
			...
			if (!reenact)
			{
				History.Database.Query("DELETE FROM build_history WHERE Account = @0 AND Time >= @1 AND " + XYReq + " AND WorldID = @2",
					account, rollbackTime, Main.worldID);
			}
			if (Main.rand == null)
